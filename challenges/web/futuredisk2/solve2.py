...
def get_bytes(offset, size=8205):
  ...
  while True:
    try:
      r = requests.get(url, headers={"Range": "bytes=" + str(start) + "-" + str(end-1)})
      assert(len(r.content) == size)
      break
    except requests.exceptions.ChunkedEncodingError:
      print("Request failed, retrying", start, end)
    except AssertionError:
      print("Request incomplete, retrying", start, end)
  return r.content
...
for i in range(b.bitlen()):
  try:
    flag = zlib.decompressobj(wbits=-zlib.MAX_WBITS).decompress(b.copy().skipBits(i).finish().bytes)
  except Exception:
